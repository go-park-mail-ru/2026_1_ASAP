...
import (
	...
	domainUser "github.com/go-park-mail-ru/2026_1_ASAP/internal/chat/domain/profile"
	...
	mock "github.com/go-park-mail-ru/2026_1_ASAP/internal/profile/usecase/contact/mock"
)
...
func TestPositiveContactsService_AddContact(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			prepare: func(f *fields) {
				f.profileRepository.EXPECT().GetProfileById(context.Background(), int64(124)).Return(&domainUser.Profile{UserId: 124, FirstName: "danil_kolbasenko"}, nil)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			s := &ContactService{
				contactRepo: f.contactRepository,
				profileRepo: f.profileRepository,
			}
...
func TestNegativeContactService_AddContact(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			prepare: func(f *fields) {
				f.profileRepository.EXPECT().GetProfileById(context.Background(), int64(124)).Return((*domainUser.Profile)(nil), domainUser.ErrNotFound)
			},
			args:    args{ctx: context.Background(), contactRequest: dto.AddContactRequest{ContactUserID: 124, FirstName: "danil_kolbasenko"}, userID: 123},
