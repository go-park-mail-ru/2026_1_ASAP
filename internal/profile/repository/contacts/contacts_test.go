...
func TestContactsRepository_GetAllContactsByUserID(t *testing.T) {
	...
	tests := []struct {
		...
		assert  func(t *testing.T, contacts []*contactdom.Contact, err error)
		name    string
		userID  int64
	}{
		{
...
func TestContactsRepository_CreateContact(t *testing.T) {
	...
	tests := []struct {
		contact *contactdom.Contact
		prepare func(m pgxmock.PgxPoolIface)
		assert  func(t *testing.T, contact *contactdom.Contact, err error)
		name    string
	}{
		{
...
func TestContactsRepository_DeleteContact(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			assert: func(t *testing.T, err error) {
				require.ErrorIs(t, err, contactdom.ErrContactNotFound)
			},
		},
		{
			name:          "db_error",
